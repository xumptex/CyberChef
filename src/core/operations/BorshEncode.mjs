...
import { strToValue,valueToStr} from "../lib/Borsh.mjs"
import * as borsh from "borsh";

/**
 * Borsh Encode operation
 */
class BorshEncode extends Operation {

    /**
     * BorshEncode constructor
     */
    constructor() {
        super();

        this.name = "Borsh Encode";
        this.module = "Default";
        this.description = `Encoding a buffer into borsh using a configurable schema : \n
...
More complex objects are described by a JSON object. The following types are supported:\n
    ...
    { struct: { field1: Schema1, field2: Schema2, ... } } - a struct. The fields of the struct are described by the field1, field2, etc. fields.`;
        ...
        this.infoURL = "https://borsh.io/"; // Usually a Wikipedia link. Remember to remove localisation (i.e. https://wikipedia.org/etc rather than https://en.wikipedia.org/etc)
    ...
    run(input, args) {
        ...
        let result = ""
        try {
            result = JSON.stringify(borsh.serialize(schema, strToValue(input)));
        }catch (e){
